...
This function finds the closest item, and moves toward it, always ending in the same distance and angel from the item.
...
- The robot moves to the item in a loop:
    ...
    - Continue the loop until the tobot is close enough to the item.
- Recongnize if the cup is standing or lying down.
...
DETECTION_DISTANCE_THRESHOLD = 60
    - The distance in cm that the robot will search for cups, won't find anything further then that.
